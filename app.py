...
def colour_qr():
    ...
    filename = f'{filename}.png'
    qrCode = segno.make(link, error='h')
    qrCode = qrCode.to_pil(
        scale=8, dark=dark, data_dark=data_dark, data_light=data_light)
    qrCode.save(f'static/created/{filename}')
    qr = Image.open(f'static/created/{filename}')
    qr = qr.resize((200, 200))
    qr.save(f'static/created/{filename}')
...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        ...
        qrcode = segno.make(link, error='h')
        qrcode.to_artistic(background=os.path.join(
            app.config['UPLOAD_FOLDER'], filename), target=target, scale=8)
        qr = Image.open(f'static/created/{filename}')
        qr = qr.resize((200, 200))
        qr.save(f'static/created/{filename}')
